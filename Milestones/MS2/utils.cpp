...
2021/07/10  Added condition to isValid() function, check if int is negative.
-----------------------------------------------------------
...
namespace sdds 
{
    ...
    bool isValid(string& str)
    {
        ...
        {
            // if string is negative int, increment counter to next position before evaluation.
            if (str[0] == '-')
            {
                i++;
            }
        ...
        const char* errorMessage, bool showRangeAtError)
    {
        ...
        while (getline(cin, str))
        {
            ...
            else
            {
                if (!isValid(str))
                {
                    if (!isdigit(str[0])) // Should still pass unit test if 0th position is negative
                    {
                        cout << "Bad integer value, try again: ";
                    }
                    else if (!ss.eof())
